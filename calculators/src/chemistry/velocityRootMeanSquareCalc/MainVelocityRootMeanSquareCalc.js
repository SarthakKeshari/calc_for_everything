...
const BoltzmannConstant = 1.380649e-23; // Boltzmann constant in J/K
const mh=1.67e-27;//mass of hydrogen
const ATOM_MASS = {
    H: 1.00794,
	He: 4.0026,
	Li: 6.941,
...
function MainVelocityRootMeanSquareCalc() {
  ...
  const [rootMeanSquareVelocity, setRootMeanSqaureVelocity] = useState('');
  ...
  function calculateMolecularWeight() {
    ...
    return finalMolecularWeight;
  }
  

  function calculateRootMeanSquareVelocity() {
    const temp = parseFloat(temperature);

    if (isNaN(temp) || !molecule) {
      setError(true);
      return;
    }
    ...
    const velocity = Math.sqrt((3 * BoltzmannConstant * temp) / (molecularWeight*mh));
    setRootMeanSqaureVelocity(velocity.toFixed(12));
    setError(false);
  }
  ...
  return (
    <Container maxWidth="lg" sx={{ bgcolor: '#eeeeee', minHeight: '90vh', paddingY: '10' }}>
      <Typography pt={1} variant="h5" sx={{ textAlign: 'center' }}>
        Velocity Root Mean Sqaure Value Calculator <InfoVelocityRootMeanSquareCalc />
      </Typography>
      <hr />
      <br />
      ...
      <Container sx={{ display: 'flex', flexDirection: 'column' }}>
        ...
        <TextField
          ...
          onChange={(e) => {
            ...
            setMolecularWeight(calculateMolecularWeight()); // Calculate and set molecular weight on input change
            setError(false);
            setRootMeanSqaureVelocity('');
          }}
          helperText={`${error ? 'Invalid Input' : ''}`}
        ...
        <Button
          variant="contained"
          onClick={calculateRootMeanSquareVelocity}
          sx={{ width: '50%', margin: 'auto', mt: '20px' }}
        >
          Calculate Root Mean Sqaure Velocity
        </Button>
      </Container>

      <hr />
      <br />
      {!error && rootMeanSquareVelocity && (
        <>
          <Typography pt={1} variant="h6" mt={2}>
          Root Mean Sqaure Velocity
          </Typography>
          <div className="output">
            <Typography pt={1} variant="h6" fontStyle={{ color: 'blue' }}>
              {rootMeanSquareVelocity} m/s
            </Typography>
          </div>
        </>
      )}
      ...
      {!error && molecularWeight && (
        <>
          ...
          <div className="output">
            <Typography pt={1} variant="h6" fontStyle={{ color: 'green' }}>
