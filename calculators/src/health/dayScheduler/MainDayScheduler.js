...
function MainDayScheduler() {
    ...
    function downloadTasks() {
        const tasksText = tasks.map(task => `Date: ${new Date().toLocaleDateString()} Task ID: ${task.taskId}, Time1: ${task.time1}, Time2: ${task.time2}, Task: ${task.task}`).join('\n');
    ...
    return (
        <Container maxWidth="lg" sx={{ bgcolor: '#f0f0f0', minHeight: '90vh', paddingY: "10" }}>
            <Container sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <Typography pt={1} variant='h5'>Day Scheduler</Typography>
                <Typography pt={1} variant='h5'>{new Date().toLocaleDateString()}</Typography>
            </Container>
            <hr />
            <br />
            <Container sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "2rem" }}>
                ...
                <Input type='time' placeholder='Enter Starting Time' value={time1} onChange={(e) => setTime1(e.target.value)} />
                <Input type='time' placeholder='Enter Ending Time' value={time2} onChange={(e) => setTime2(e.target.value)} />
